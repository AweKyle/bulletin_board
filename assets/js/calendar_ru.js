...
document.write('<table id="fc" style="z-index:1;position:absolute;border-collapse:collapse;background:#FFFFFF;border:1px solid #ddd;display:none;-moz-user-select:none;-khtml-user-select:none;user-select:none;" cellpadding="2">');
...
document.write('<tr style="background:#0073AA;font:12px Arial;color:#FFFFFF"><td align=center>П</td><td align=center>В</td><td align=center>С</td><td align=center>Ч</td><td align=center>П</td><td align=center>С</td><td align=center>В</td></tr>');
...
document.write('<tr><td colspan="7" align="center" style="cursor:pointer;font:13px Arial;background:#84B8D1" onclick="today()">Сегодня: '+addnull(sccd,sccm+1,sccy)+'</td></tr>');
...
function lcs(ielem) {
	updobj=ielem;
	/*getObj('fc').style.left=Left(ielem)+'449px';
	getObj('fc').style.top=Top(ielem)+ielem.offsetHeight+'-45px';*/
	getObj('fc').style.left='287px';
	/*getObj('fc').style.top='-115px';*/
...
function cs_over(e) {
	evtTgt(EvtObj(e)).style.background='#84B8D1';
}
function cs_out(e) {
...
function f_cps(obj) {
	...
	obj.style.border='1px solid #ddd';//'1px solid #606060';
	...
	for(var d=1;d<=42;d++) {
		...
		if ((d >= (cd -(-1)))&&(d<=cd-(-marr[cm]))) {
			...
			if (sccm == cm && sccd == (d-cd) && sccy == cy)
				cv.style.color='#0073AA';

			// if selected date
			if (cm == selectedm && cy == selectedy && selectedd == (d-cd) )
			{
				cv.style.background='#84B8D1';
